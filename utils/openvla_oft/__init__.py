...
def get_img_embedding(vla, images, language_embeddings=None, use_film=False):
    ...
    pixel_values = images.to(torch.bfloat16).to(vla.device)

    if use_film:
        patch_features = vla.vision_backbone(pixel_values, language_embeddings)  # Ensure the backbone supports this interface
    else:
