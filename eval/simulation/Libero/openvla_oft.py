...
OPENVLA_ROOT = os.path.join(PROJECT_ROOT, "openvla_oft")
...
class GenerateConfig:
    ...
    pretrained_checkpoint: Union[
        str, Path] = "moojink/openvla-7b-oft-finetuned-libero-object"  # Pretrained checkpoint path
    ...
    local_log_dir: str = "./logs/oft-ad"  # Local directory for eval logs
    ...
    # fmt: on
    patch_attack: bool = True
    perturbation_primary_path: str = ""
    perturbation_wrist_path: str = ""
...
def prepare_observation(obs, resize_size, perturbation_primary, perturbation_wrist, top, left, top_wrist, left_wrist):
    ...
    img_resized = resize_image_for_policy(perturbed__primary_img, resize_size)
    wrist_img_resized = resize_image_for_policy(perturbed_wrist_img, resize_size)
...
def eval_libero(cfg: GenerateConfig) -> float:
    ...
    # Load perturbation
    if cfg.patch_attack:
        if cfg.perturbation_primary_path == "" or cfg.perturbation_wrist_path == "":
            raise ValueError(
                "If using patch-based attack, please provide valid perturbation paths for both primary and wrist!")
        else:
            # Load primary patch
            if cfg.perturbation_primary_path.endswith(".npy"):
                perturbation_primary = torch.from_numpy(np.load(cfg.perturbation_primary_path))
            elif cfg.perturbation_primary_path.endswith(".pt"):
                perturbation_primary = torch.as_tensor(torch.load(cfg.perturbation_primary_path))
            else:
                raise ValueError("Unsupported primary perturbation file type! Use .npy or .pt")
            # Load wrist patch
            if cfg.perturbation_wrist_path.endswith(".npy"):
                perturbation_wrist = torch.from_numpy(np.load(cfg.perturbation_wrist_path))
            elif cfg.perturbation_wrist_path.endswith(".pt"):
