...
def eval_libero(cfg: GenerateConfig) -> None:
    ...
    if cfg.patch_attack:
        if cfg.perturbation_path == "":
            ...
            elif cfg.perturbation_path.endswith(".pt"):
                perturbation = torch.load(cfg.perturbation_path)
            else:
                raise ValueError("Unsupported perturbation file type! Use .npy or .pt")
    # Start evaluation
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
            ...
            # # [Prompt Attack] Perturb task description
            # attacked_description = attacker.perturb(task, task_description, initial_states, episode_idx)
            ...
            # print(f"\nAttacked Task: {attacked_description}")
            # log_file.write(f"\nAttacked Task: {attacked_description}\n")
            ...
            while t < max_steps + cfg.num_steps_wait:
                try:
                    ...
                    if cfg.patch_attack:
                        img = apply_perturbation_to_raw_images([img], perturbation, (top, left))[0]
